...
#---------------------------------------------------------------------------------
all: lib/libnotifications.a

dist-bin: all
	@tar --exclude=*~ -cjf libnotifications-$(VERSION).tar.bz2 include lib

dist-src:
	@tar --exclude=*~ -cjf libnotifications-src-$(VERSION).tar.bz2 include source Makefile

dist: dist-src dist-bin

install: dist-bin
	mkdir -p $(DESTDIR)$(DEVKITPRO)/wums
	bzip2 -cd libnotifications-$(VERSION).tar.bz2 | tar -xf - -C $(DESTDIR)$(DEVKITPRO)/wums
...
lib/libnotifications.a :$(SOURCES) $(INCLUDES) | lib release
